...
public class Ex_03_문제004 {
	// 힌트
	// - D[i][1] = D[i-1][1] + A[i][1]
	// - D[i][j] = D[i][j-1] + D[i-1][j] - D[i-1][j-1] + A[i][1]

	public static void main(String[] args) throws IOException {
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		StringTokenizer st = new StringTokenizer(br.readLine()); // N(배열크기), M(질의 수)
		...
		for(int i = 1; i <= N; i++) {
			st = new StringTokenizer(br.readLine()); // 원본 배열 값
			for(int j = 1; j <= N; j++) {
		...
		for(int i = 0; i < M; i++) {
			st = new StringTokenizer(br.readLine()); // 질의 계산
