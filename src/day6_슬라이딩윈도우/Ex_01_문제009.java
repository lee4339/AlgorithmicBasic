...
// 슬라이딩 윈도우 알고리즘
// : 2개의 포인터로 범위를 지정한 다음 범위를 유지한 채로 이동하여 문제 해결
...
public class Ex_01_문제009 {
	...
	public static void main(String[] args) throws IOException {
		...
		for(int i = 0; i < P; i++) { // 부분 문자열 처음 받을 때 세팅 - 초기 P 부분 문자열 처리 부분
			Add(A[i]);
		}
		...
		for(int i = P; i < S; i++) {
			...
			Add(A[i]);
			Remove(A[j]);
			if(checkSecret == 4) Result++; // 4자릿수와 관련된 크기가 모두 충족될 때 유요한 비밀번호
		}
